...
| `replays.test.mjs` | (#42) every sample in `tests/replays/` migrates, validates and plays to its recorded result; a sample and a migration exist for every version that ever was (v1 per game, v2 an Isolation win against the Normal bot); the `bad-*` files stay refused with a reason; `fromRecord` round trips a finished and an unfinished game, trims the config to the game's own fields and carries the bot's level and budget (`meta.bot`); file name (`.minigames.replay`), date and summary with its `bot` flag; the pure list filter by game, bot or not, name and local day range; the store (newest first, per game, one entry per game, in memory under jsdom); `Match.watch` |
...
| `premove.test.mjs` | (#37) set / switch / take back, the board covered while a timed online game's clock is paused (and never without a timer or for a spectator), played through the normal path when the turn comes, dropped once it went illegal, never on one device or as a spectator, cleared by a new game / stop / the end, and the one clicked while the bot thinks |
...
| `winchance.test.mjs` | frozen while a move animates, then refined stage by stage; the smoothing; the estimator's choice (a bot with `evaluate` wins over the rules' `estimate`, calibration applied); Dots and Boxes reading the same number at every stage |
...
| `i18n.test.mjs` | (#47) every language file carries exactly the English keys with the same placeholders per key and plural form and no dashes; the module (`t`, plurals, `msg` with nested descriptors, `apply`, `detect`, `init` and `dir`); every text and attribute in `index.html` keyed to the English it shows; every bot and difficulty with its key; and the lint that finds a user-facing string literal left in `client/**/*.js` |
...
`overflows(selectors[, max])` (#47: every visible element the selectors match whose text takes
more than `max` lines or is wider than its box, each text node measured on its own),
...
| `settings` | the board size clamped per game, five's minimum following the win length (and its list only offering sizes that fit), the Yavalath rule played out in a local game, every control being a dropdown with its Custom row (no checkboxes, no bare numbers), the chain rule writing both of its config keys, everything surviving a reload, and a timer that only runs for the player to move and pauses during animations |
...
| `mobile` | 360 × 780: the title screen and join panel, the local lobby and the settings modal, an online-shaped lobby with four seats in every look (one Invite button beside the code and the four labelled share rows stacked in its modal, the seat controls inside their seat cards, the seat count on the head's line with the Players heading over the seat cards carrying the spectator count, and no hairlines left, the two groups holding the right controls, seat names never cut off, the screen scrolling to Start while nothing scrolls inside the card, screenshots `mobile-lobby-<skin>.png`), the game with its compact HUD and visible board outline, the stacked result overlay, the replay bar above the HUD (#38) and the Isolation board with a step picked |
...
| `boxes` | the Dots and Boxes card, the shared "Board size" row and its summary, the board of dots / lines / boxes, a closed box that keeps you on turn, a whole seeded game to the last line with the overlay texts, the replay bar, a rematch, three on one device, 360 × 780 without scrolling, and the bot drawing lines and closing boxes |
...
| `online-bot` | **three browsers** (#36): alone in a two-seat room, "Against a bot instead" puts a bot on the empty seat, it plays (with no "Waiting for your friend" banner over the game, #52) and a spectate-link viewer sees its moves and reactions, a friend joining mid-game watches and gets the seat when the bot steps aside in the lobby, and the bot comes back when the friend leaves and keeps playing across a refresh of the host |
| `i18n` | (#47) the language following the browser, the switch in the preferences changing every text and surviving a reload, Arabic right to left with the board left to right, and for every language on a 360 × 780 phone and a 1400 × 900 desktop a walk over every screen, modal, menu and the learn ladders asserting through `overflows` that no single-line text wraps or overflows (see "Languages" in `ui.md` for what may take two lines) and that the phone screens do not scroll; one assertion per walk names every text that broke |
