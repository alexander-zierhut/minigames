/* Bot persona: a bot seat reacts with emojis now and then so it feels like a player (#14).
   ...
   start, "GG" at the end, "EZ" once when its win chance jumps, 🚨 for a strong human
   move (the bot's chance drops a lot), 😲 for a move the bot considers among the worst
   options, 😔 once when it thinks it is losing — never more than one reaction every
   COOLDOWN_MS and MAX_PER_GAME per game. Seeded (Bots.rng from the bot's seed), so a
...
const BotPersona = (() => {
    ...
    // maybe say something: once per key when `once`, respecting cooldown, cap and probability
    function say(emoji, probability, { key, once = true } = {}) {
        const s = session;
    ...
    function judge(s, cell, after) {
        ...
        if (before && !before.over) {
            ...
            if (legal.length >= 6) {
                ...
                if (drop >= 0.15 && better <= Math.ceil(alternatives * 0.1) && s.counts.alert < 2) { if (say("🚨", 0.7, { key: `alert${s.counts.alert}` })) s.counts.alert++; }
                else if (worse === 0 && better >= Math.floor(alternatives * 0.75) && mine - prev >= 0.1 && s.counts.surprise < 2) { if (say("😲", 0.6, { key: `surprise${s.counts.surprise}` })) s.counts.surprise++; }
            }
        }
        if (!after.over && mine >= 0.9) say("EZ", 0.7, { key: "ez" });
        else if (!after.over && mine <= 0.12) say("😔", 0.7, { key: "sad" });
    }
    ...
    function onFinish(ev) {
        ...
        say("GG", 0.9, { key: "gg" });
        if (won) say("😄", 0.5, { key: "happy" });
        else if (ev.winner >= 0) say("😔", 0.4, { key: "lost" });
    }

    return { attach, detach, get active() { return session; }, COOLDOWN_MS, MAX_PER_GAME };
})();
