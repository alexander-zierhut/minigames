...
async function showRoomLobby() {
    await M.ev("document.getElementById('lobby-code').textContent = 'ABCDE'; document.getElementById('lobby-share').hidden = false; document.getElementById('lobby-players').hidden = false; document.getElementById('lobby-spectators').textContent = '2 spectators watching'; document.getElementById('btn-watch').hidden = false; true");
}
test("online-shaped lobby with four seats: one Share button plus two icons under the code, no scroll, every skin", async () => {
    ...
    for (const skin of ["classic", "mcboard", "mc"]) {
        ...
        const swap = JSON.parse(await M.ev("JSON.stringify(['btn-watch', 'lobby-players', 'lobby-spectators'].map(id => { const r = document.getElementById(id).getBoundingClientRect(); return { top: Math.round(r.top), left: Math.round(r.left), right: Math.round(r.right) }; }))"));
        assert.ok(swap[0].top >= swap[1].top && swap[0].top <= swap[2].top, `${skin}: seat controls between the cards and the notes (${JSON.stringify(swap)})`);
        assert.ok(swap[0].left >= 16 && swap[0].right <= 344, `${skin}: seat controls inside the card`);
