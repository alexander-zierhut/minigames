...
const { Bots, Rules } = H;
const CONFIGS = { chain: { n: 5, chainRule: false }, five: { n: 7, winLen: 5 }, boxes: { n: 3 } };
...
test("registry: every bot has valid metadata; bad definitions are rejected", () => {
    ...
    assert.equal(Bots.get("random-chain").baseline, true); assert.equal(Bots.get("random-five").baseline, true); assert.equal(Bots.get("random-boxes").baseline, true);
    assert.equal(Bots.botFor("chain").id, "creeper-chain"); assert.equal(Bots.botFor("five").id, "sensei-five"); assert.equal(Bots.botFor("boxes").id, "fencer-boxes");
