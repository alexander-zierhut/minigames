/* Chat + log: lines in the HUD log with the sender's colour (the lobby has no chat, #15),
   limits (length, rate), text only, nothing sent offline, Bus events, chat survives a
   new game. */
...
test("log: newest first, MAX_LINES kept, chat lines in the HUD log only, HTML never interpreted", () => {
    const { w, d, L } = setup();
    ...
    const el = d.getElementById("log").firstChild;
    assert.equal(el.className, "chat p1");
    ...
    assert.equal(d.getElementById("lobby-log"), null, "no lobby log any more (#15)");
    assert.equal(d.getElementById("lobby-chat-input"), null, "no lobby chat input any more (#15)");
    assert.equal(L.room, undefined, "room events went with the lobby log");
    w.close();
});
...
test("offline nothing is sent and the row is disabled; the game log keeps chat across a new game", () => {
