...
test("sections (#32): the menu rows summarise their state, showSection opens one, Back returns", () => {
    ...
    assert.equal(P.sectionSummary("streaming"), "on", "both options on = content creator mode");
    ...
    assert.equal(P.get().hideCode, false); assert.equal(P.get().privateIp, false);
    assert.equal(P.sectionSummary("streaming"), "Best for streaming");
    ...
    d.getElementById("pref-creator-mode").click();
    assert.equal(P.get().hideCode, true); assert.equal(P.get().privateIp, true);
