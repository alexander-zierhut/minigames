...
test("defaults: quiet volume, every sound category on, sound set follows the look", () => {
    ...
    assert.equal(p.muteSpectators, false, "spectators are heard unless asked");
...
test("sections (#32): the menu rows summarise their state, showSection opens one, Back returns", () => {
    ...
    P.set({ hideCode: true });
    assert.equal(P.sectionSummary("streaming"), "partly on");
    P.set({ privateIp: true });
    assert.equal(P.sectionSummary("streaming"), "partly on", "two of three");
    P.set({ muteSpectators: true });
    assert.equal(P.sectionSummary("streaming"), "on", "every option on = content creator mode");
    assert.equal(d.getElementById("pref-creator-mode").textContent, "Turn off content creator mode");
    assert.equal(d.getElementById("pref-creator-mode").classList.contains("primary"), false, "the off button is secondary");
    d.getElementById("pref-creator-mode").click();
    assert.equal(P.get().hideCode, false); assert.equal(P.get().privateIp, false); assert.equal(P.get().muteSpectators, false);
    assert.equal(P.sectionSummary("streaming"), "Best for streaming");
    assert.equal(d.getElementById("pref-creator-mode").textContent, "Turn on content creator mode");
    assert.equal(d.getElementById("pref-creator-mode").classList.contains("primary"), true);
    d.getElementById("pref-creator-mode").click();
    assert.equal(P.get().hideCode, true); assert.equal(P.get().privateIp, true); assert.equal(P.get().muteSpectators, true);
