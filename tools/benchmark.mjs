...
for (const def of bots) {
    ...
    if (old) { try { const o = JSON.parse(old[1]); if (o.score === result.score && o.games === result.games && o.version === result.version && o.avgMoves === result.avgMoves) { result.at = o.at; result.commit = o.commit; } } catch (e) { /* rewrite */ } }
    writeFileSync(file, `/* generated by tools/benchmark.mjs — do not edit */\nBots.benchmark("${def.id}", ${JSON.stringify(result)});\n`);
    console.log(`${def.id.padEnd(16)} ${String(result.score).padStart(5)} % vs ${baselineId} over ${series.games} games (${Math.round((Date.now() - t0) / 1000)} s)`);
}
